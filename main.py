...
def return_borrowed_book():
    ...
    cur.execute(f'''UPDATE {table_name} SET status='available', date='{datetime.date.today()}' 
                        WHERE first_name='{first_name}' AND last_name='{last_name}' AND book='{book_name}' AND status='borrowed';''')
    con.commit()
